...
uint8_t RemoteIP[] = {192,168,9,187};
    ...
    if(Socket != -1)
    {
      ...
      if(ret == WIFI_STATUS_OK)
      {
        if(Datalen > 0)
        {
          ...
          memcpy(pCutXYZ, pDataXYZ, sizeof(pDataXYZ));
          TERMOUT("Measured: (%d, %d, %d)\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);
          fflush(stdout);
          TERMOUT("IN byte:(%x,%x,%x,%x,%x,%x)\n", pCutXYZ[0], pCutXYZ[1], pCutXYZ[2], pCutXYZ[3], pCutXYZ[4], pCutXYZ[5]);
          fflush(stdout);
          ret = WIFI_SendData(Socket, pCutXYZ, sizeof(pCutXYZ), &Datalen, WIFI_WRITE_TIMEOUT);
